...
void TM1637TinyDisplay6::writeBuffer()
{
  ...
  if(m_flipDisplay) {
    ...
    for (uint8_t k=0; k < MAXDIGITS; k++) {
      ...
      */
      //RTH LISYclock HW 1.00 reverse, ( 123456 shows 321654 )      
      writeByte(digitsbuf[3]); 
      writeByte(digitsbuf[4]); 
      writeByte(digitsbuf[5]); 
      writeByte(digitsbuf[0]); 
      writeByte(digitsbuf[1]); 
      writeByte(digitsbuf[2]); 
