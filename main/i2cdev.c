...
esp_err_t i2c_dev_init(i2c_dev_t *dev)
{
	...
	my_esp_err = i2c_new_master_bus(&i2c_mst_config, &i2c_bus_handle);
	if (my_esp_err != ESP_OK) return(my_esp_err);
	...
	my_esp_err = i2c_master_bus_add_device(i2c_bus_handle, &dev_cfg, &i2c_dev_handle);
	if (my_esp_err != ESP_OK) return(my_esp_err);

	//probe if device is connected
	my_esp_err = i2c_master_probe(i2c_bus_handle, DS3231_ADDR, 100);
	if (my_esp_err != ESP_OK) return(my_esp_err);
