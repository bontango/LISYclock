...
//
esp_err_t CheckFWUpdate(char *fname) {
    ...
    if (stat(fname, &st) != 0) {
        ...
        ESP_LOGI(TAG, "found new firmware");
        return ESP_OK;
    }
}
...
//    ESP_LOGI(TAG, "ota result = %d", err);
    ...
    // Delete update file to prevent endless loop    
    unlink(fname);
    ESP_LOGI(TAG, "update file deleted");
